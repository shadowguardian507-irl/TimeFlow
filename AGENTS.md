...
3. Load any additional rule-detail files required by those workflow loader files before executing the relevant
   workflow phase.

## Rule Priority

Rules loaded from `.ai-rules/core-workflow-load.md` have the highest repository-level priority.
Rules loaded from `.ai-rules/aidlc-workflow-load.md` are next and apply to all software development requests unless a
